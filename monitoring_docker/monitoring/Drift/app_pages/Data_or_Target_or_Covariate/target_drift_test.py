...
def app():
    ...
    if st.button("get test result"):
        ...
        new_data_frame0 = pipe_dnd.fit_transform(new_data_frame)
        reference_data_frame0 = new_data_frame0.copy().iloc[2:3]
        ...
        # Create target frame from new and reference frame
        new_data_frame = pd.DataFrame(
            new_data_frame0["survived"].values.tolist(), columns=["survived"]
        )
        reference_data_frame = pd.DataFrame(
            reference_data_frame0["survived"].values.tolist(), columns=["survived"]
        )
        ...
        # Get the list of numerical and categorical variable column names from
        # reference_data and new_data in order
