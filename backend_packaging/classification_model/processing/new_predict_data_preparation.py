...
# ===== FUNCTION TO DROP TARGET VARIABLE IN PREDICT DATA SET =====
def drop_target_variable(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        if "survived" in predict_raw_data.columns:
            return predict_raw_data.drop(["survived"], axis=1)
        else:
...
def replace_missing_by_nan_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    predict_raw_data = input_data.copy()
    exclude_target_variable = drop_target_variable(input_data=predict_raw_data)
    ...
    ).transform(exclude_target_variable)
...
# Create configuration for persisiting user data set.
# Features used here are in the data set witch gives by users (_ui = user input),
# data set witch gives to the pipeline on training or test step (_mi = model input),
# and model prediction
class TitanicAllDataInputAndOutputSchema(BaseModel):
    pclass: Optional[int]
    name: Optional[str]
    sex: Optional[str]
    age: Optional[float]
    ...
    body: Optional[float]
    homedest: Optional[str]
    pclass_mi: Optional[float]
    age_mi: Optional[float]
    sibsp_mi: Optional[float]
    parch_mi: Optional[float]
    fare_mi: Optional[float]
    age_na_mi: Optional[float]
    fare_na_mi: Optional[float]
    age_outliers_mi: Optional[float]
    sibsp_outliers_mi: Optional[float]
    parch_outliers_mi: Optional[float]
    fare_outliers_mi: Optional[float]
    sex_female_mi: Optional[float]
    cabin_Rare_mi: Optional[float]
    cabin_Missing_mi: Optional[float]
    cabin_B_mi: Optional[float]
    embarked_S_mi: Optional[float]
    embarked_Q_mi: Optional[float]
    title_Mrs_mi: Optional[float]
    title_Miss_mi: Optional[float]
    title_Mr_mi: Optional[float]
    survived: Optional[float]


class MultipleTitanicAllDataInputAndOutput(BaseModel):
    inputs: List[TitanicAllDataInputAndOutputSchema]
